...
- `readiness` 组应显式包含 `db`、`redis`，否则它可能不足以表达“外部依赖已就绪”
...
- 构建阶段使用 Node.js
- 运行阶段提供静态资源服务
- 若构建环境受限，可改用“宿主机构建 dist，容器仅封装运行时”的方式
...
- 本地容器联调时可直接访问 `127.0.0.1:3000` 和 `127.0.0.1:3001`
- 宝塔 Nginx 仍然是生产入口层，不影响正式部署职责划分
- 能降低入口代理临时失配时的排障复杂度
...
5. 在本地完成镜像构建，再上传 / 导入到生产服务器

原因：

- 生产服务器规格为 2 核 2 GB，不适合作为三端镜像的主要构建节点
